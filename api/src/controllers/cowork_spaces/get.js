...
const getAllCoworkSpaces = async () => {
  ...
  allCoworkSpaces.map((cowork) => {
    cowork.score =
      cowork.reviews.reduce((acc, review) => acc + review.score, 0) /
      cowork.reviews.length;
    return cowork;
  });
  return allCoworkSpaces;
};

const getCoworkSpacesBySearch = async (
  name,
  score,
  location,
  services,
  ...
  let coworkSpaceByName = await CoworkSpace.find(filters).populate(
    "reviews",
    "score"
  );
  coworkSpaceByName = coworkSpaceByName.map((cowork) => {
    ...
    newCowork.score = newCowork.reviews.length
      ? newCowork.reviews.reduce((acc, review) => {
          return acc + review.score;
        }, 0) / newCowork.reviews.length || 1
      : 0;
  ...
  if (score) {
    coworkSpaceByName = coworkSpaceByName.filter(
      (cowork) => cowork.score === parseInt(score)
    );
  }
