                ...
                <div className ="shadow-lg rounded-lg hover:scale-110 transition duration-300" key={card._id}>
                  ...
                  <Swiper navigation={true} modules={[Navigation]} className="mySwiper">
                   ...
                   <div className='p-5'>
                    ...
                    <p className='text-lg font-normal text-gray-600 dark:text-white line-clamp-5'>{card.about} </p><Link href={`/detail/${card.id}`}>Ver mas</Link>
    ...
    <div className="flex items-center">
      {stars.map((star, idx) => ( 
        <svg 
          key = {idx}
          aria-hidden = "true"
          className = {`w-5 h-5 ${star <= idx ?  'text-gray-300' : 'text-yellow-400' } mr-2 mb-2 mt-3 `}
          fill = "currentColor"
          viewBox='0 0 20 20'
