...
const ReviewForm = () => {
  ...
  const renderStars = (selectedScore) => {
    ...
    for (let i = 1; i <= 5; i++) {
      const starClass =
        i <= selectedScore ? "text-yellow-500" : "text-gray-300";
      stars.push(
        <FontAwesomeIcon
          key={i}
          icon={faStar}
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="mt-8 flex-grow mx-auto p-4 bg-white rounded shadow max-w-md">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            <label className="block font-medium mb-2  dark:text-black">
              Puntuación
            </label>
      ...
      <Footer />
      {showModal && (
        <Modal onClose={closeModal} content="Nueva reseña creada" />
      )}
    </div>
  );
};
